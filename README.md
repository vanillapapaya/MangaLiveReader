...
**합성 화면이다.** 만화는 저작물이라 저장소에 넣지 않으므로(맨 아래 「이용 범위」)
칸과 말풍선을 직접 그렸고 대사도 지어냈다. **오버레이는 진짜다** —
`extension/overlay.css` 를 그대로 불러 쓰고 DOM 구조도 `content.js` 가 만드는 것과
같다. 원본은 `docs/demo.html`, 찍는 것은 `docs/shoot.sh` (윈도우 크롬 헤드리스).

노란 테두리가 번역이 도착한 말풍선, 점선이 말풍선 밖 글씨(효과음)다. 왼쪽 위가
상태줄, 오른쪽 위가 버튼 패널 — 켜져 있는 「자동」이 주황색이다.
