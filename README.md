...
그래픽카드가 없거나 NVIDIA 가 아니어도 돌아가긴 한다. 다만 글자 인식이 CPU 로
떨어져 실사용이 어렵다 — 런처가 미리 경고하고 계속할지 물어본다.
...
| 의존성 | 5GB 쯤 (`.venv` 실측 4.8GB) |
| 모델 가중치 | 77MB |
| API 키 | 없으면 창에서 물어보고 저장 |

**첫 실행은 그만큼 받느라 오래 걸린다.** 회선에 달렸으니 켜 두고 다른 일을 하는
편이 낫다. 두 번째부터는 전부 「있음」으로 건너뛴다.
...
NVIDIA 가 아닌 기계에서 이 인덱스를 그대로 타면 쓰지도 못할 CUDA 런타임을 몇 GB 씩
받는다 (WSL venv 에서 `nvidia/` 만 4.2GB 였다).
