...
**그림만 손으로 그렸다.** 만화는 저작물이라 저장소에 넣지 않으므로(맨 아래 「이용
범위」) 칸·말풍선·인물을 직접 그렸다. **그 위에 얹힌 것은 전부 실제 결과다:**

| | |
|---|---|
| 노란 박스의 위치 | comic-text-detector 가 낸 bbox 그대로 |
| 일본어 원문 | manga-ocr 이 읽은 것 (6곳 전부 맞았다) |
| 한국어 라벨 | `gemini-3.6-flash` 가 번역한 것 |
| 「쿵」의 점선 박스 | 모델이 효과음(sfx)으로 분류한 것 |
| 왼쪽 위 숫자 | 그때 실제로 걸린 시간 — 검출 584ms · OCR 4519ms · 번역 8122ms |

다시 만들려면 `docs/shoot.sh` 와 `docs/run_demo.py` 다 (GPU·API 키 필요).
오버레이는 `extension/overlay.css` 를 그대로 불러 쓰고 DOM 구조도 `content.js` 가
만드는 것과 같다.
...
이쪽은 **실물 그대로다.** 같은 기계에 띄웠으면 손댈 것이 없고, 다른 기기에서 붙을
때만 주소와 토큰을 넣는다.
...
**만화가 아닌 페이지에서는 잘 안 된다.** 청공문고 『羅生門』 페이지로 재봤다 —
검출이 **2개**뿐이었다. 제목은 정확히 읽었지만(「羅生門 芥川龍之介」) 본문은
900×719 한 덩어리로 잡혀 OCR 이 `．．．` 를 뱉었다. manga-ocr 은 말풍선 한 조각을
읽도록 학습된 모델이라 산문 한 페이지를 주면 그렇게 된다. 검출기도 마찬가지로
만화 칸을 전제한다.
