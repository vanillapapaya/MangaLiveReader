...
class ReadMeta:
    ...
    #: 뷰어 사각형(자르기 전)을 **전송 이미지 좌표**로 적은 것. `[x, y, w, h]`.
    #:
    #: 캐시가 좌표를 되돌리는 기준이다. phash 는 퍼지 매칭이라 크기가 다른 캡처도
    #: 같은 행에 붙는데, 저장된 bbox 는 그때 캡처의 좌표계라 그대로 얹으면 어긋난다.
    #: 이 값을 같이 저장해 두면 나중 캡처에서 비율로 환산해 제자리에 놓을 수 있다.
