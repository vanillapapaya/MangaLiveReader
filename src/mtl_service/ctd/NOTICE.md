...
따라서 이 디렉터리의 코드는 **GPL-3.0** 이다.

**정리 완료.** 저장소 전체를 GPL-3.0-or-later 로 공개한다 (루트 `LICENSE`). 서비스가
이 디렉터리를 직접 import 하므로(`detect.py`) 다른 라이선스를 고를 수 없다 — 떼려면
별 프로세스로 분리해야 하는데 개인 도구에 그만한 값이 없다.

가중치(`comictextdetector.pt`)는 **저장소에 넣지 않는다.** `scripts/fetch_models.py`
가 원 배포처에서 받는다.
